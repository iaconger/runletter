...
import { getMyProfile, listExtras, listScheduled } from "@/lib/db/programs";
...
export default async function Home({ searchParams }: { searchParams: Promise<{ kind?: string; m?: string }> }) {
  const { kind, m } = await searchParams;
  ...
  const [mine, theirs, dropped] = me
    ? await Promise.all([listExtras(me.id, from, to), followedDays(from, to).catch(() => []), listScheduled(from, to).catch(() => [])])
    : [[], [], []];

  const byDateMine = new Map<string, typeof mine>();
  for (const x of mine) byDateMine.set(x.date, [...(byDateMine.get(x.date) ?? []), x]);
  const byDateTheirs = new Map<string, typeof theirs>();
  for (const d of theirs) byDateTheirs.set(d.date, [...(byDateTheirs.get(d.date) ?? []), d]);
  ...
  const monthRuns = mine.filter((x) => RUNS.includes(x.sportType) && x.date.slice(0, 7) === iso(first).slice(0, 7));
