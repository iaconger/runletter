...
export default async function CalendarPage() {
  ...
  const rows = buildWeeks({
    ...
    doneIds: cal?.doneIds ?? new Set(), extras, sentWeeks: cal?.sentWeeks, hideUnsent: true,
  ...
  const km = recent.reduce((a, x) => a + (x.distanceM ?? 0), 0) / 1000;
    ...
    <main className="rl-page rl-wide rl-stack" style={{ maxWidth: "calc(1100px + 2 * var(--rl-gutter))", gap: "var(--rl-space-5)" }}>
      <div className="rl-between" style={{ alignItems: "baseline", flexWrap: "wrap", gap: "var(--rl-space-2)" }}>
        ...
        <div className="rl-row" style={{ gap: 6 }}>
          {recent.length > 0 && <span className="rl-chip">Last 30 days · {runs} run{runs === 1 ? "" : "s"} · {km.toFixed(0)} km</span>}
      ...
      <span className="rl-help">Colour is the run type. A tick is done. <em>+km</em> and the other sports are from your Strava.</span>
    </main>
  );
}
