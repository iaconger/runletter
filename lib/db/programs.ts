...
export function mapProfile(r: Pick<ProfileRow, "id" | "handle" | "display_name" | "avatar_url" | "cover_url" | "bio" | "is_creator" | "links"> & { pace_5k_s?: number | null; stripe_charges_enabled?: boolean | null; goal?: Program["goal"] | null; race_date?: string | null; days_per_week?: number | null; strava_stats?: unknown; strava_gear?: unknown; list_publicly?: boolean; units?: string | null }): Profile {
  ...
  return { id: r.id, handle: r.handle, displayName: r.display_name, avatarUrl: r.avatar_url, coverUrl: r.cover_url, bio: r.bio, isCreator: r.is_creator,
    pace5kS: r.pace_5k_s ?? null, stripeChargesEnabled: r.stripe_charges_enabled ?? false, goal: r.goal ?? null, raceDate: r.race_date ?? null, daysPerWeek: r.days_per_week ?? null, stravaStats: r.strava_stats ?? null, stravaGear: r.strava_gear ?? null, listPublicly: r.list_publicly ?? true, units: r.units === "mi" ? "mi" : "km", links };
}
...
const MY_PROFILE_COLS = "id, handle, display_name, avatar_url, cover_url, bio, is_creator, links, pace_5k_s, stripe_charges_enabled, goal, race_date, days_per_week, strava_stats, strava_synced_at, units, strava_gear, list_publicly";
...
export type PublicFollower = { id: string; handle: string; name: string; avatarUrl: string | null; since: string };

/** Who is running with a creator, for their public page. Only runners who allow being listed. */
export async function listCreatorFollowers(creatorId: string, limit = 24): Promise<PublicFollower[]> {
  const supabase = await createClient();
